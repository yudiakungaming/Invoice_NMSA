...
window.handleFileSelect = function(input) {
  ...
  // Validasi ukuran file (max 750KB per file)
  const maxSize = 750 * 1024;
  const oversized = files.filter(f => f.size > maxSize);

  if (oversized.length > 0) {
    showToast(`File ${oversized[0].name} terlalu besar! Maksimal 750KB.`, 'error');
    input.value = '';
    return;
  }
  ...
  files.forEach(function(file, index) {
    ...
    reader.onload = function(e) {
      state.selectedFiles.push({
        ...
        type: file.type,
        size: file.size,
        base64: e.target.result.split(',')[1], // Remove data:mime;base64, prefix
      ...
      if (processed === files.length) {
        updateFileDisplay();
        console.log('[Files] Ready:', state.selectedFiles.length, 'files');
      }
    };
...
function updateFileDisplay() {
  ...
  if (state.selectedFiles.length > 0) {
    ...
    display.innerHTML = state.selectedFiles.map(function(file) {
      const sizeKB = (file.size / 1024).toFixed(1);
      const base64SizeKB = Math.round((file.base64?.length || 0) * 0.75 / 1024);
      ...
      return `
        <div class="file-item">
          ...
          <span class="text-green-400 text-xs truncate flex-1">${escapeHtml(file.name)}</span>
          <span class="text-gray-500 text-xs">${sizeKB} KB</span>
          <span class="drive-badge">${base64SizeKB}KB</span>
        </div>
      `;
    }).join('');
  } else {
...
async function handleSubmit(e) {
  ...
  try {
    ...
    // Convert files to Base64 format for Firestore
    let uploadedFiles = [];
    
    if (state.selectedFiles.length > 0) {
      elements.submitBtn.innerHTML = 
        `<div class="spinner"></div><span>Mengkonversi ${state.selectedFiles.length} file ke Base64...</span>`;
    ...
    updateDriveStatusBanner(
      'pending',
      '⏳ Menunggu sync ke Spreadsheet... (Klik "Sync Sekarang" untuk langsung!)'
    );
...
function loadAppsScriptCode() {
  ...
  // Kode Apps Script (disederhanakan)
  const appsScriptCode = `/**
 * FinanceSync Pro v3.4 - Google Apps Script Backend
 * Copy kode ini ke script.google.com
 */
...
function doPost(e) {
  var result = syncPendingSubmissions();
  return ContentService.createTextOutput(JSON.stringify({
    success: true,
    ...result
  })).setMimeType(ContentService.MimeType.JSON);
...
window.previewFromHistory = async function(docId) {
  ...
  try {
    ...
    const previewData = {
      ...
      disetujui_oleh: data.disetujui_oleh || DEFAULT_SIGNATORIES.disetujui_olej || DEFAULT_SIGNATORIES.disetujui_oleh,
  ...
  elements.historyList.innerHTML = filtered.slice(0, 30).map(function(item) {
    ...
    if (item.files && item.files.length > 0) {
      ...
      if (syncedCount === item.files.length && item.files.length > 0) {
        syncIndicator = '<span class="sync-indicator synced" title="Semua file synced ke Google Drive">✓ Synced</span>';
